    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium mt-1">Audit previous warehouse replenishment records</p>
        </div>
        <Link
          href="/purchases/new"
      ...
      <div className="bg-slate-900/40 border border-slate-800/80 p-4 rounded-2xl shadow-xl flex items-center gap-4">
        <div className="flex-1 relative">
          ...
          <input
            type="text"
            placeholder="Search by Invoice Number, Supplier name, or reference code..."
            value={searchQuery}
              ...
              {filteredInvoices.length > 0 ? (
                filteredInvoices.map((inv) => (
                  <tr key={inv.id} className="hover:bg-slate-900/20 transition">
                    ...
                    <td className="px-6 py-4">
                      <div className="font-bold text-slate-200">{inv.supplier.name}</div>
                      <div className="text-[10px] text-slate-500">{inv.supplier.companyName || 'No Company'}</div>
                    </td>
                ...
                <tr>
                  <td colSpan={9} className="px-6 py-12 text-center text-slate-500">
                    No purchase history invoices recorded.
                  </td>
                </tr>
              )}
            </tbody>
                    ...
                    <div>
                          ...
                          <table style={{ borderCollapse: 'collapse', fontSize: '9px', width: '200px', marginLeft: 'auto', border: '1px solid #1e3a8a' }}>
                            <tbody>
                              ...
                              <tr>
                                ...
                                <td style={{ border: '1px solid #1e3a8a', padding: '4px 6px', textAlign: 'right', color: '#0f172a' }}>{activeInvoice.referenceNumber || 'N/A'}</td>
                              </tr>
                            </tbody>
                          </table>
                        </div>
                      ...
                      <div style={{ marginBottom: '20px' }}>
                        ...
                        <div style={{ padding: '8px 4px', fontSize: '10px', color: '#334155', lineHeight: '1.4' }}>
                          <strong style={{ color: '#0f172a', fontSize: '11px' }}>{activeInvoice.supplier.name}</strong><br />
                          {activeInvoice.supplier.companyName && <span>Company: {activeInvoice.supplier.companyName}<br /></span>}
                          {activeInvoice.supplier.phone && <span>Phone: {activeInvoice.supplier.phone}<br /></span>}
                          {activeInvoice.supplier.address && <span>Address: {activeInvoice.supplier.address}<br /></span>}
                        </div>
                      </div>
                        ...
                        <tbody>
                          {activeInvoice.items.map((item, idx) => (
                            <tr key={item.id} style={{ color: '#334155' }}>
                              ...
                              <td style={{ border: '1px solid #1e3a8a', padding: '5px', textAlign: 'right' }}>${item.purchasePrice.toFixed(2)}</td>
                              <td style={{ border: '1px solid #1e3a8a', padding: '5px', textAlign: 'right', fontWeight: 'bold', color: '#0f172a' }}>${item.total.toFixed(2)}</td>
                            </tr>
                          ))}
                        ...
                        <table style={{ borderCollapse: 'collapse', fontSize: '10px', width: '220px', border: '1px solid #1e3a8a' }}>
                          <tbody>
                            <tr>
                              ...
                              <td style={{ border: '1px solid #1e3a8a', padding: '4px 6px', textAlign: 'right', color: '#0f172a' }}>${(activeInvoice.grandTotal - activeInvoice.shippingCharges - activeInvoice.taxAmount + activeInvoice.discount).toFixed(2)}</td>
                            </tr>
                            <tr>
                              ...
                              <td style={{ border: '1px solid #1e3a8a', padding: '4px 6px', textAlign: 'right', color: '#0f172a' }}>+${activeInvoice.taxAmount.toFixed(2)}</td>
                            </tr>
                            {activeInvoice.discount > 0 && (
                              <tr>
                                ...
                                <td style={{ border: '1px solid #1e3a8a', padding: '4px 6px', textAlign: 'right', color: '#dc2626' }}>-${activeInvoice.discount.toFixed(2)}</td>
                              </tr>
                            )}
                            <tr>
                              ...
                              <td style={{ border: '1px solid #1e3a8a', padding: '4px 6px', textAlign: 'right', color: '#0f172a' }}>+${activeInvoice.shippingCharges.toFixed(2)}</td>
                            </tr>
                            <tr>
                              ...
                              <td style={{ border: '1px solid #1e3a8a', padding: '4px 6px', textAlign: 'right', color: '#16a34a', fontWeight: 'bold' }}>${activeInvoice.paidAmount.toFixed(2)}</td>
                            </tr>
                            <tr>
                              ...
                              <td style={{ border: '1px solid #1e3a8a', padding: '4px 6px', textAlign: 'right', color: activeInvoice.dueAmount > 0 ? '#dc2626' : '#475569', fontWeight: 'bold' }}>${activeInvoice.dueAmount.toFixed(2)}</td>
                            </tr>
                            <tr style={{ backgroundColor: '#1e3a8a', color: '#ffffff' }}>
                              <td style={{ border: '1px solid #1e3a8a', padding: '5px 6px', fontWeight: 'bold', textAlign: 'left' }}>TOTAL COST</td>
                              <td style={{ border: '1px solid #1e3a8a', padding: '5px 6px', textAlign: 'right', fontWeight: 'black' }}>${activeInvoice.grandTotal.toFixed(2)}</td>
                            </tr>
                          </tbody>
                        </table>
                      </div>
