...
export default function SuppliersPage() {
  ...
  const loadSuppliers = async () => {
    ...
    try {
      if (typeof window !== 'undefined' && window.electron) {
        const list = await window.electron.invoke('parties:list-suppliers');
        setSuppliers(list || []);
        setFiltered(list || []);
      }
    } catch (err) {
  ...
  const handleCreateSupplier = async (e: React.FormEvent) => {
    ...
    try {
      if (typeof window !== 'undefined' && window.electron) {
        const res = await window.electron.invoke('parties:create-supplier', {
          name,
          phone,
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium mt-1">Manage wholesale suppliers and replenishment profiles</p>
        </div>
        <button
      ...
      <div className="bg-slate-900/40 border border-slate-800/80 p-4 rounded-2xl shadow-xl flex items-center gap-4">
        <div className="flex-1 relative">
          ...
          <input
            type="text"
            placeholder="Search suppliers by name, phone number, or company..."
            value={search}
          ...
          filtered.map((s) => (
            ...
            >
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="font-extrabold text-sm text-slate-200">{s.name}</h3>
                  <span className="text-[10px] text-slate-500 mt-0.5 block font-mono">Company: {s.companyName || 'No Company'}</span>
                </div>
              ...
              <div className="border-t border-slate-800 pt-3 flex justify-between items-center text-[9px] text-slate-500">
                <span>ACCOUNT LEDGER LINKED</span>
                <span className="font-bold text-slate-400 font-mono">SUP-{s.name.slice(0,3).toUpperCase()}</span>
              </div>
            </motion.div>
          ))
        ) : (
          <div className="col-span-full py-12 text-center text-slate-500 text-xs">
            No supplier profiles matched the filters.
          </div>
        )}
      </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            ...
            >
              <div className="flex justify-between items-center border-b border-slate-800 pb-3">
                <h3 className="font-extrabold text-sm text-slate-200">Register Supplier Profile</h3>
              ...
              <form onSubmit={handleCreateSupplier} className="space-y-4">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 px-1">Supplier Contact Name *</label>
                  <input
                    type="text"
                    required
                    value={name}
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 px-1">Company Title</label>
                  <input
                    type="text"
                    value={companyName}
